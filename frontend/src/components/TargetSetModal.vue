  ...
  <div v-if="isOpen" class="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
    <div class="dark-panel w-full max-w-lg p-6 space-y-6 border border-emerald-500/40 shadow-2xl">
      ...
      <div class="space-y-4">
        <!-- Meta Mensal Input -->
        <div>
          <label class="block text-xs uppercase text-slate-500 font-semibold mb-1">Meta Mensal (Unidades)</label>
          <input
        ...
        <div class="grid grid-cols-2 gap-4 bg-slate-50 p-4 rounded-xl border border-slate-200">
          <div>
            <span class="text-[11px] text-slate-500 font-semibold block uppercase">Meta Semanal (Calculada)</span>
            <span class="text-base font-mono font-bold text-slate-900">{{ calculatedWeekly }} un/semana</span>
          </div>

          <div>
            <span class="text-[11px] text-slate-500 font-semibold block uppercase">Meta Diária (Calculada)</span>
            <span class="text-base font-mono font-bold text-emerald-600">{{ calculatedDaily }} un/dia</span>
          </div>
        </div>

        <div>
          <label class="block text-xs uppercase text-slate-500 font-semibold mb-1">Produto a Produzir</label>
          <input
            v-model="product"
            type="text"
            placeholder="Ex: Bolacha, Chocolate..."
            class="w-full bg-slate-50 border border-slate-300 text-slate-900 rounded-lg p-3 text-sm focus:border-emerald-500 focus:outline-none font-semibold"
          />
        </div>

        <div>
          <label class="block text-xs uppercase text-slate-500 font-semibold mb-1">Código do Lote</label>
          <input
            v-model="lot"
            type="text"
            placeholder="Ex: G0227B"
            class="w-full bg-slate-50 border border-slate-300 text-emerald-600 font-mono font-bold rounded-lg p-3 text-sm focus:border-emerald-500 focus:outline-none"
          />
        </div>
      </div>
