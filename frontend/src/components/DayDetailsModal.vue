<template>
  <div v-if="isOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-fadeIn">
    <div class="bg-white border border-slate-200 rounded-2xl w-full max-w-4xl max-h-[90vh] flex flex-col shadow-2xl overflow-hidden font-sans">
      
      <!-- Modal Header -->
      <div class="p-5 border-b border-slate-200 flex items-center justify-between bg-slate-50">
        <div class="flex items-center gap-3">
          <div class="w-10 h-10 rounded-xl bg-emerald-500/10 border border-emerald-500/30 flex items-center justify-center text-emerald-600 font-bold">
            📅
          </div>
          <div>
            <h2 class="text-lg font-extrabold text-slate-900 uppercase tracking-wider">
              Detalhamento de Apontamentos & Ocorrências
            </h2>
            <p class="text-xs text-emerald-600 font-semibold font-mono">
              {{ formattedDate }}
              <span v-if="shiftLabel" class="text-slate-500 font-sans"> · Turno: <span class="text-amber-600">{{ shiftLabel }}</span></span>
        ...
        <button 
          @click="$emit('close')"
          class="w-9 h-9 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-500 hover:text-slate-900 flex items-center justify-center transition-colors text-lg">
          ✕
        </button>
      </div>

      <!-- Loading State -->
      <div v-if="loading" class="p-12 text-center text-slate-500 space-y-3 flex-1 flex flex-col items-center justify-center">
      ...
      <div class="p-4 border-t border-slate-200 bg-slate-50 flex justify-end">
        <button 
          @click="$emit('close')"
          class="px-5 py-2 bg-slate-100 hover:bg-slate-200 text-slate-900 font-bold text-xs rounded-xl uppercase tracking-wider transition-all">
          Fechar
        </button>
      </div>
