  ...
  <div class="min-h-screen bg-[#f1f5f9] text-slate-900 font-sans flex select-none">
    ...
    <main class="flex-1 p-4 pt-[calc(4rem+env(safe-area-inset-top))] md:p-6 md:pt-6 lg:p-8 overflow-y-auto max-w-7xl mx-auto w-full">
      ...
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 md:mb-8">
        <div>
          <h1 class="text-2xl sm:text-3xl font-extrabold uppercase tracking-wider text-slate-900">CENTRAL DE ALERTAS</h1>
        ...
        <div class="flex items-center gap-3 self-start sm:self-auto">
          <button
            ...
            class="px-4 py-2 bg-slate-100 hover:bg-slate-200 border border-slate-300 text-slate-700 text-xs font-bold rounded-lg uppercase transition-all flex items-center gap-2 disabled:opacity-50"
          >
      ...
      <div v-if="store.possibleStops.length" class="space-y-4 mb-6 md:mb-8">
        <p class="text-xs text-amber-600 uppercase tracking-wider font-bold flex items-center gap-1.5">
        ...
        >
          <div class="flex items-start gap-3">
            <div class="w-10 h-10 rounded-xl bg-orange-500/10 border-2 border-orange-500 text-orange-600 flex items-center justify-center text-lg shrink-0">⚠️</div>
            <div>
              <p class="text-orange-600 font-black text-sm uppercase tracking-wider">
          ...
          <div class="flex flex-col sm:flex-row gap-3">
            <select
              ...
              class="flex-1 bg-white border border-slate-300 rounded-lg px-3 py-3 text-sm text-slate-900 focus:border-orange-500 focus:outline-none"
            >
            ...
            <button
              ...
              class="px-6 py-3 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-sm uppercase tracking-wider rounded-xl transition-all active:scale-95 shrink-0 disabled:opacity-40"
            >
              Não é parada
            </button>
          </div>
      ...
      <div v-else-if="store.errors.alerts" class="dark-panel p-6 border border-red-500/30 text-center">
        <p class="text-red-600 font-semibold">⚠ Erro ao buscar alertas: {{ store.errors.alerts }}</p>
      ...
      <div v-else-if="!store.alerts.length" class="dark-panel p-12 text-center space-y-3">
        ...
        <p class="text-emerald-600 font-bold text-lg">Nenhum alerta em aberto</p>
        <p class="text-slate-500 text-sm">Sistema operando normalmente.</p>
      </div>
        ...
        >
          <div class="flex flex-col sm:flex-row items-start justify-between gap-4">
            <div class="space-y-1.5 flex-1">
              ...
              <p class="text-sm text-slate-700 font-medium">{{ alert.message || alert.descricao }}</p>
            </div>
            ...
            <button
              ...
              :class="alert.status === 'acknowledged'
                ...
                : 'bg-emerald-500/20 text-emerald-600 border border-emerald-500/50 hover:bg-emerald-500/30'"
            >
