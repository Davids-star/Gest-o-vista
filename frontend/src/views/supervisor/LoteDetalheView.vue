<template>
  <div class="min-h-screen bg-[#f1f5f9] text-slate-900 flex select-none">
    <AppSidebar />
    ...
    <main class="flex-1 p-4 pt-[calc(4rem+env(safe-area-inset-top))] md:p-6 md:pt-6 lg:p-8 overflow-y-auto flex flex-col items-center justify-center">
      <div class="w-full max-w-lg dark-panel p-8 space-y-6 border border-emerald-500/30">
        ...
        <div class="flex items-center gap-4 border-b border-slate-200 pb-4">
          ...
          <div>
            <h1 class="text-xl font-extrabold uppercase tracking-widest text-slate-900">LOTE</h1>
            <p class="text-sm text-slate-500">
              Estação
              <span class="text-emerald-600 font-bold font-mono text-base">{{ currentMachineDisplayName }}</span>
            </p>
          </div>
          <div class="ml-auto font-mono text-emerald-600 font-bold text-sm bg-slate-50 border border-slate-200 px-3 py-1.5 rounded-lg">
            {{ currentTime }}
          </div>
        </div>
        ...
        <div v-if="store.loading.machines" class="text-center py-8">
          ...
          <p class="text-slate-500 text-sm">Carregando dados da estação...</p>
        </div>

        <!-- Lote Ativo (da sessão ou da store) -->
        <template v-else>
          <section class="bg-slate-50 rounded-xl p-4 border border-slate-200 space-y-3">
            <div class="flex items-center justify-between">
              <h2 class="text-xs font-bold uppercase tracking-widest text-slate-700">Definir lote sem iniciar produção</h2>
              <button @click="showLotForm = !showLotForm" class="px-3 py-1.5 bg-emerald-500/20 border border-emerald-500/40 text-emerald-600 rounded-lg text-xs font-bold">{{ showLotForm ? 'FECHAR' : 'NOVO LOTE' }}</button>
            </div>
            <div v-if="showLotForm" class="grid gap-3">
              <select v-model="lotForm.machine_id" class="w-full bg-white border border-slate-300 rounded-lg p-3 text-sm">
              ...
              <select v-model="lotForm.product_id" class="w-full bg-white border border-slate-300 rounded-lg p-3 text-sm">
              ...
              <div class="flex gap-2">
                <input v-model.trim="lotForm.code" class="flex-1 bg-white border border-slate-300 rounded-lg px-3 py-3 text-sm" placeholder="Código do lote">
              ...
              <p v-if="lotCreateError" class="text-xs text-red-600">{{ lotCreateError }}</p>
            </div>
          </section>
          <div>
            <label class="block text-xs uppercase text-slate-500 font-bold mb-2 flex items-center gap-2">
          ...
          <div v-if="selectedSession" class="bg-slate-50 rounded-xl p-4 border border-slate-200 text-xs space-y-1.5">
            <p class="text-slate-500 font-semibold uppercase tracking-wider">Sessão Ativa</p>
            <div class="flex justify-between">
              <span class="text-slate-500">Máquina</span>
              <span class="font-semibold text-emerald-600">{{ currentMachineDisplayName }}</span>
            </div>
            <div class="flex justify-between" v-if="selectedSession.operator?.name">
              <span class="text-slate-500">Operador</span>
              <span class="text-slate-900 font-semibold">{{ selectedSession.operator.name }}</span>
            </div>
          </div>

          <div v-else class="bg-amber-500/10 border border-amber-500/30 rounded-xl p-3 text-xs text-amber-600 font-semibold">
            ⏳ Nenhuma sessão ativa nesta estação.
            Inicie via Apontamento do supervisor.
          </div>

          <!-- Erro ao salvar -->
          <p v-if="saveError" class="text-red-600 text-xs font-semibold">⚠ {{ saveError }}</p>
          ...
          <div v-if="!isEditing" class="flex gap-3">
            ...
            <button
              @click="goBack"
              class="flex-1 py-3 rounded-xl bg-slate-100 hover:bg-slate-200 border border-slate-400 text-slate-700 font-bold text-sm uppercase tracking-wider transition-all flex items-center justify-center gap-2 active:scale-[0.98]"
            >
              <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <Transition name="edit-actions">
            <div v-if="isEditing" class="flex gap-3">
              <button
                @click="cancelEdit"
                class="flex-1 py-3 rounded-xl bg-slate-100 hover:bg-slate-200 border border-slate-400 text-slate-700 font-bold text-sm uppercase tracking-wider transition-all active:scale-[0.98]"
              >
                CANCELAR
              </button>
              <button
