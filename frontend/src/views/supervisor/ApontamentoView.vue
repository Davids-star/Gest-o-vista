  ...
  <div class="min-h-screen bg-[#f1f5f9] text-slate-900 font-sans flex select-none">
    ...
    <main class="flex-1 p-4 pt-[calc(4rem+env(safe-area-inset-top))] md:p-6 md:pt-6 lg:p-8 overflow-y-auto max-w-7xl mx-auto w-full">
      ...
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 md:mb-8">
        <div>
          <h1 class="text-2xl sm:text-3xl font-extrabold uppercase tracking-wider text-slate-900">
            APONTAMENTO <span class="text-sm font-normal text-emerald-600">por estação</span>
          </h1>
        </div>
        ...
        <div class="flex items-center gap-3 self-start sm:self-auto">
          ...
          <!-- Relógio ao Vivo -->
          <div class="bg-white border border-slate-200 px-4 py-2 rounded-xl text-slate-700 font-mono text-sm flex items-center gap-2">
      ...
      <section class="mb-6">
        <h2 class="text-xs font-bold uppercase tracking-widest text-slate-500 mb-3 flex items-center justify-between">
        ...
        <div v-else-if="!store.machines.length" class="dark-panel p-6 text-center text-slate-500">
          ...
          >
            <div class="flex items-center justify-between gap-2 mb-1">
              <div class="flex items-center gap-2.5 min-w-0">
                <span class="w-8 h-8 rounded-lg border border-emerald-500/40 bg-emerald-500/10 text-emerald-600 flex items-center justify-center font-mono font-extrabold text-xs shrink-0">
                  {{ getMachineNumber(st) }}
                </span>
                <div class="min-w-0">
            ...
            <div class="mt-2 pt-2 border-t border-slate-200 flex items-center justify-between text-[10px]">
              <span 
                class="inline-flex items-center gap-1 font-bold uppercase"
                :class="isMachineOperating(st.id) ? 'text-emerald-600' : 'text-slate-500'">
      ...
      <div v-if="!selectedMachine && store.machines.length" class="dark-panel p-8 text-center text-slate-500">
      ...
      <template v-if="selectedMachine">
        ...
        <div class="bg-white border border-slate-200 rounded-2xl p-4 md:p-5 mb-6 flex flex-wrap items-center justify-between gap-4">
          <div class="flex items-center gap-3">
            <div class="w-12 h-12 rounded-xl bg-emerald-500/10 border border-emerald-500/30 flex items-center justify-center text-emerald-600 font-mono font-black text-lg">
              {{ getMachineNumber(selectedMachine) }}
            </div>
            <div>
              <h2 class="text-lg font-extrabold text-slate-900 uppercase tracking-wider flex items-center gap-2">
                {{ getMachineDisplayName(selectedMachine) }}
                <span class="text-xs text-slate-500 font-normal">({{ selectedMachine.name || selectedMachine.code }})</span>
              </h2>
              <p class="text-xs text-slate-500">
                Acompanhando produção exclusiva de <strong class="text-slate-900">{{ getMachineDisplayName(selectedMachine) }}</strong> no dia de hoje
              </p>
            </div>
          </div>
          ...
          <div class="flex items-center gap-2">
            <span class="px-3 py-1 rounded-full text-xs font-bold border uppercase"
                  :class="selectedSession ? 'bg-emerald-500/10 border-emerald-500/30 text-emerald-600' : 'bg-amber-500/10 border-amber-500/30 text-amber-600'">
        ...
        <section class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
          ...
          <div class="dark-panel p-5 space-y-2 border-emerald-500/30">
            <div class="flex items-center justify-between">
              <span class="text-xs font-bold uppercase tracking-wider text-slate-500">📦 Produção Feita</span>
              <span class="text-[10px] bg-emerald-500/10 text-emerald-600 border border-emerald-500/30 px-2 py-0.5 rounded font-bold uppercase">Esta Máquina</span>
            </div>
            <div class="flex items-baseline gap-2">
              <span class="font-mono text-3xl md:text-4xl font-black text-emerald-600">
                {{ selectedProduction.toLocaleString('pt-BR') }}
              </span>
              <span class="text-xs text-slate-500">unidades hoje</span>
            </div>
          </div>
          ...
          <div class="dark-panel p-5 space-y-2 border-blue-500/30">
            <div class="flex items-center justify-between">
              <span class="text-xs font-bold uppercase tracking-wider text-slate-500">🟢 Tempo Produzido</span>
          ...
          <div class="dark-panel p-5 space-y-2 border-red-500/30">
            <div class="flex items-center justify-between">
              <span class="text-xs font-bold uppercase tracking-wider text-slate-500">🔴 Tempo Parado</span>
              <span class="text-[10px] bg-red-500/10 text-red-600 border border-red-500/30 px-2 py-0.5 rounded font-bold uppercase">Em Parada</span>
            </div>
            <div class="flex items-baseline gap-2">
              <span class="font-mono text-3xl md:text-4xl font-black text-red-600">
          ...
          <div class="dark-panel p-6 space-y-5">
            <div class="flex items-center justify-between border-b border-slate-200 pb-3">
              ...
              <span
                ...
                :class="selectedSession ? 'text-emerald-600 border-emerald-500/40 bg-emerald-500/10' : 'text-slate-500 border-slate-300 bg-slate-100'"
              >
            ...
            <div v-if="selectedSession" class="bg-white rounded-xl p-4 border border-slate-200 space-y-2.5 text-xs">
              <div class="flex justify-between items-center">
                <span class="text-slate-500">Estação Selecionada:</span>
                <span class="font-bold text-emerald-600 font-mono">{{ getMachineDisplayName(selectedMachine) }}</span>
              </div>
              <div class="flex justify-between items-center" v-if="selectedSession.product?.name">
                <span class="text-slate-500">Produto em Fabricação:</span>
                <span class="font-bold text-slate-900">{{ selectedSession.product.name }}</span>
              </div>
              <div class="flex justify-between items-center" v-if="selectedSession.lot?.code">
                <span class="text-slate-500">Lote Ativo:</span>
                <span class="font-mono font-bold text-slate-900 bg-slate-100 px-2 py-0.5 rounded">{{ selectedSession.lot.code }}</span>
              </div>
              <div class="flex justify-between items-center" v-if="selectedSession.operator?.name">
                <span class="text-slate-500">Operador:</span>
                <span class="font-semibold text-slate-900">{{ selectedSession.operator.name }}</span>
              </div>
              <div class="flex justify-between items-center" v-if="selectedSession.started_at">
                <span class="text-slate-500">Início da Operação:</span>
                <span class="font-mono text-slate-700">{{ formatHora(selectedSession.started_at) }}</span>
              </div>
            </div>

            <div v-else class="bg-slate-50 rounded-xl p-4 border border-slate-200 text-xs text-slate-500 space-y-2">
              <p>Nenhuma sessão ativa para <strong>{{ getMachineDisplayName(selectedMachine) }}</strong> no momento.</p>
              <p class="text-[11px] text-slate-500">Clique em <strong class="text-emerald-600">INICIAR SESSÃO</strong> para disparar o apontamento desta máquina.</p>
            </div>
          ...
          <div class="dark-panel p-6 space-y-4">
            <div class="flex items-center justify-between border-b border-slate-200 pb-3">
              ...
              <button
                ...
                class="px-3 py-1.5 bg-red-500/20 border border-red-500/40 hover:bg-red-500/30 text-red-600 rounded-lg text-xs font-bold flex items-center gap-1.5 transition-all"
              >
            ...
            <div class="overflow-x-auto min-h-[140px]">
              <div v-if="!paradasHoje.length" class="text-slate-500 text-xs text-center py-8 bg-slate-50 rounded-xl border border-slate-200">
              ...
              <table v-else class="w-full text-left text-xs border-collapse">
                ...
                <tbody class="divide-y divide-slate-800/60 font-mono">
                  <tr v-for="stop in paradasHoje" :key="stop.id" class="hover:bg-slate-100">
                    ...
                    <td class="py-2.5 font-sans">
                      <span class="font-semibold text-amber-600">{{ stop.reason?.label || stop.motivoParada?.label || 'Não especificado' }}</span>
                    </td>
